...
def init_users_db():
    if not os.path.exists(USERS_FILE):
        ...
        # التأكد من وجود حساب المسؤول الجديد في قاعدة البيانات
...
def install_req(folder):
    ...
    # تشغيل عملية التثبيت في الخلفية
    try:
        proc = subprocess.Popen(
        ...
        return jsonify({"success": True, "message": "تم بدء التثبيت، راجع السجل للتفاصيل"})
...
def proxy(port, path):
    """
    تمرير الطلبات إلى السيرفرات المحلية
    مثال: /proxy/8000/api/spam?user_id=12345678
    """
    try:
        port = int(port)
        ...
        # بناء الرابط الكامل مع الـ Query String
        query_string = request.query_string.decode('utf-8')
        url = f"http://localhost:{port}/{path}"
        if query_string:
            url += f"?{query_string}"
        
        # تمرير الرأس والبيانات
        ...
        if request.method == "GET":
            ...
            resp = req_lib.put(url, data=request.get_data(), headers=headers, timeout=30)
        elif request.method == "DELETE":
            resp = req_lib.delete(url, headers=headers, timeout=30)
        elif request.method == "PATCH":
            resp = req_lib.patch(url, data=request.get_data(), headers=headers, timeout=30)
